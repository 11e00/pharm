...
import {supabase,API} from "@/app/db";

export default async function CategoryPage({ params }: { params: { category: string } }) {
    const urlCategoryName = params.category;

    let categoryName=(await API(supabase.from('Category').select('category_name').eq('category_name', urlCategoryName)))[0]?.category_name?? [];
    
    if(urlCategoryName!=categoryName)
        notFound();

    let categoryID=(await API(supabase.from('Category').select('category_id').eq('category_name', urlCategoryName)))[0]?.category_id ?? [];
