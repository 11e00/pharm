...
export default async function ProductPage({params}:{params:{product:string}}) {
    ...
    const productID:number=(await API(supabase.from('item').select('item_id').eq('item_id', urlProductID)))[0]?.item_id;

    if(urlProductID!=productID.toString())
        notFound();

    const product:item[]=await API(supabase.from('item').select('*').eq('item_id', productID)) ?? [];

    const item_images:item_images[]=await API(supabase.from('item_images').select('*').eq('item_id', productID)) ?? [];
